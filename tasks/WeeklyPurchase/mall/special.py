...
class Special(Buy, MallNavbar):
    ...
    def execute_special(self, con: SpecialRoom = None):
        ...
        if not con.enable:
            logger.info('[商店-特殊] 特殊商店未启用，跳过')
            return
        if not self._enter_special():
            logger.warning('[商店-特殊] 进入特殊商店失败')
            return
        # 向下滑找到购买的物品
        ...
        while 1:
            ...
            if totem_bought and medium_bought and low_bought:
                logger.info('[商店-特殊] 本周商品已全部购买')
                break
            if self.appear(self.I_SP_SWIPE_CHECK):
                # 如果滑动到底了
                logger.info('[商店-特殊] 滑动到页面底部')
                break
    ...
    def _special_totom(self, totem_pass: bool):
        ...
        if not totem_pass:
            logger.info('[商店-特殊] 未启用购买图腾，跳过')
            return
        # 检查剩余数量
        buy_number = 40
        remain_number = self._special_check_remain(self.I_SP_BUY_TOTEM)
        if remain_number < buy_number:
            logger.warning(f'[商店-特殊] 图腾数量不足: {remain_number}')
            if remain_number <= 0:
                logger.warning('[商店-特殊] 图腾余量识别为空')
                return
    ...
    def _special_medium(self, buy_number: int = 10):
        ...
        if buy_number <= 0:
            logger.info('[商店-特殊] 未启用购买中号契约书，跳过')
            return
        # 检查剩余数量
        remain_number = self._special_check_remain(self.I_SP_BUY_MEDIUM)
        if remain_number < buy_number:
            logger.warning(f'[商店-特殊] 中号契约书数量不足: {remain_number}')
            if remain_number <= 0:
                logger.warning('[商店-特殊] 中号契约书余量识别为空')
                return
            buy_number = remain_number
        # 用金币买的没必要再检查金币
        # 购买 , 中级盘一次最多购买20个
        buy_max = 20
        logger.info(f'[商店-特殊] 实际购买 {buy_number} 次')
    ...
    def _special_low(self, buy_number: int = 10):
        ...
        logger.hr('Buy low', 3)
        if buy_number <= 0:
            logger.info('[商店-特殊] 未启用购买小号契约书，跳过')
            return
        # 检查剩余数量
        remain_number = self._special_check_remain(self.I_SP_BUY_LOW)
        if remain_number < buy_number:
            logger.warning(f'[商店-特殊] 小号契约书数量不足: {remain_number}')
            if remain_number <= 0:
                logger.warning('[商店-特殊] 小号契约书余量识别为空')
                return
            buy_number = remain_number
        # 用金币买的没必要再检查金币
        # 购买 , 低级级盘一次最多购买20个
    ...
    def _special_check_remain(self, target: RuleImage):
        ...
        logger.info(f'[商店-特殊] 当前余量 [{result}]')
        return result

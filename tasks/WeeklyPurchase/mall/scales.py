...
class Scales(Buy, MallNavbar):
    ...
    def _scales_orochi_new(self, buy_number: int):
        ...
        if buy_number == 0:
            logger.info('[鳞片商店] 八岐大蛇鳞片购买数量为0，跳过')
            return
        self.screenshot()
        # 检查是否出现了购买按钮
        if not self.appear(self.I_SCA_OROCHI_SCALES):
            logger.warning('[鳞片商店] 未找到八岐大蛇鳞片商品')
            return
        while True:
            self.screenshot()
            # 检查剩余数量
    ...
    def _scales_orochi(self, buy_number: int):
        ...
        remain_number = self.O_SCA_NUMBER_OROCHI.ocr(self.device.image)
        if remain_number == 0:
            logger.warning(f'[鳞片商店] 剩余购买次数异常: {remain_number}')
            return
        if remain_number < buy_number:
            buy_number = remain_number
            logger.warning(f'[鳞片商店] 剩余购买次数 {remain_number}，计划购买 {buy_number}')
        # 检查钱是否够
        cu, res, total = self.O_SCA_RES_OROCHI.ocr(self.device.image)
        if cu + res != total:
            logger.warning('[鳞片商店] 余量识别失败')
        ...
        if not money_enough:
            logger.warning('[鳞片商店] 金币不足，无法购买八岐大蛇鳞片')
            # 判断够不够买2个
            if cu < 100:
                logger.warning('[鳞片商店] 金币不足两份，按一份购买')
                return
        # 购买
        if not money_enough:
            self._scales_buy_more(self.I_SCA_OROCHI_SCALES)
    ...
    def _scales_demon(self, buy_number: int, buy_class: DemonClass=DemonClass.ODOKURO, buy_position: int=1):
        ...
        if buy_number == 0:
            logger.info('[鳞片商店] 魍魉之匣鳞片购买数量为0，跳过')
            return
        self.screenshot()
        # 检查是否出现了购买按钮
        if not self.appear(self.I_SCA_DEMON_SOULS):
            logger.warning('[鳞片商店] 未找到魍魉之匣鳞片商品')
            return
        # 检查剩余数量
        ...
        if not money_enough:
            logger.warning('[鳞片商店] 金币不足，无法购买魍魉之匣鳞片')
            # 判断够不够买2个
        ...
        # 选择购买的御魂和位置
        logger.info(f'[鳞片商店] 购买 {buy_class} 鳞片 {buy_number} 次，层数 {buy_position}')
        match_class = {
    ...
    def _scales_sea(self, buy_number: int, buy_rule: str='auto'):
        ...
        if buy_number == 0:
            logger.info('[鳞片商店] 永生之海鳞片购买数量为0，跳过')
            return
        self.screenshot()
        # 检查是否出现了购买按钮
        if not self.appear(self.I_SCA_PICTURE_BOOK):
            logger.warning('[鳞片商店] 未找到永生之海鳞片商品')
            return
        # 检查剩余数量
        ...
        if not money_enough:
            logger.warning('[鳞片商店] 金币不足，无法购买永生之海鳞片')
            # 判断够不够买2个
        ...
        # 购买# 一次最多可以买10个所以要分开来
        logger.info(f'[鳞片商店] 购买永生之海鳞片 {buy_number} 次')
        if buy_number >= 10:

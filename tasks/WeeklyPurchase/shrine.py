...
class Shrine(GameUi, WeeklyPurchaseAssets):
    ...
    def _check_bought(self, target) -> bool:
        ...
        if '已' in result or '兑' in result or '换' in result:
            logger.info('[神社] 本周已购买')
            return True
        logger.info('[神社] 本周未购买')
        return False

...
class Guild(Buy, GameUi, WeeklyPurchaseAssets):
    ...
    def _guild_honor_gift(self):
        ...
        if number == 0:
            logger.warning('[寮商店] 没有可购买的神秘的护符')
            return False
    ...
    def _guild_skin_ticket(self, num: int = 0):
        ...
        if num == 0:
            logger.warning('[寮商店] 未启用购买皮肤券，跳过')
            return False
        self.screenshot()
    ...
    def check_remain(self, image: RuleImage) -> int:
        ...
        logger.info(f'[寮商店] 图像识别区域 {image.roi_front}')
        logger.info(f'[寮商店] 余量识别区域 {self.O_GUILD_REMAIN.roi}')
        self.screenshot()
